...
public class MPHttpClientCaller {
    ...
    public static String doReusableHttpGet(String url,HttpErrorResponseHandler handler){
        ...
        while(tryAgain){
            ...
            if(mpHttpClient.isIoSuccess()){
                ...
                plan=handler.handleHttpErrorResponse(req,mpHttpClient.getResponseStatus()); 

            }else {//网络不正常，则有两种可能，一种发送请求前出现异常。一种是读取返回数据时出了异常

                if(mpHttpClient.getResponseStatus()>0){//读取返回数据时出了异常

                    plan=handler.handleIOExceptionInResponseProcess(req,mpHttpClient.getIoStatus()); 

                }else{//发送请求前出现异常

                    plan=handler.handleIOExceptionBeforeResponse(req,mpHttpClient.getIoStatus()); 
    ...
    public static JSONObject doDisposableHttpGetJsonObj(String url){
        return JSONObject.parseObject(doDisposableHttpGet(url));
    }
    
    public static JSONArray doDisposableHttpGetJsonArray(String url){
